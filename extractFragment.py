...
def get_neg_frag(fasta_dict,positive_dict,idlist,nb_windows, empty_aa,focus):
    seq_list_2d = []
    id_list = []
    pos_list = []
    focus_list = []
...
def extractFragforPredict(fasta_file,windows=16,empty_aa="-",focus=("S","T","Y")):
    ...
    frag,ids,poses,focus = get_neg_frag(fasta_dict,positive_dict,idlist, windows, empty_aa,focus)
  
    idList = ids[0].apply(lambda x: x[4:10]) 
    result = pd.concat([idList, poses, frag], axis=1)
    #result = result.apply(lambda x: ','.join(x.dropna().astype(str).values), axis=1) operation too expensive
    np.savetxt("negative_frag.txt", result, fmt='%s')
    return frag,ids,poses,focus


extractFragforPredict("processed_fasta.fasta", 16,"-","S,T,Y")